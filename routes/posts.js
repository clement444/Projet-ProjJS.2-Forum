...
router.get('/', (req, res) => {
    ...
    if (filter === 'category' && category_id) {
        posts = query(`
...
router.get('/post/create', requireAuth, (req, res) => {
    const categories = query('SELECT id, name FROM categories ORDER BY name');
    res.render('create_post', { username: req.session.username, categories, error: null });
});
...
router.post('/post/create', requireAuth, upload.single('image'), (req, res) => {
    ...
    for (const catId of categoryIds) {
        run('INSERT OR IGNORE INTO post_categories (post_id, category_id) VALUES (?, ?)', [postId, catId]);
    }
...
router.get('/post/:id', (req, res) => {
    ...
    if (!id || id <= 0) return res.status(404).render('error', { code: 404, message: "La page que vous cherchez n'existe pas." });
    ...
    if (!post) return res.status(404).render('error', { code: 404, message: "La page que vous cherchez n'existe pas." });
    ...
    for (const comment of comments) {
        const cl = queryOne(`
            SELECT
                SUM(CASE WHEN value=1 THEN 1 ELSE 0 END) as likes,
                SUM(CASE WHEN value=-1 THEN 1 ELSE 0 END) as dislikes
            FROM likes WHERE comment_id = ?`, [comment.id]);
...
router.get('/post/edit', requireAuth, (req, res) => {
    ...
    const categories = query('SELECT id, name FROM categories ORDER BY name');
    const selectedRows = query('SELECT category_id FROM post_categories WHERE post_id = ?', [id]);
    const selected = {};
...
router.post('/post/edit', requireAuth, (req, res) => {
    ...
    run('UPDATE posts SET title = ?, content = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [title, content, post_id]);
    run('DELETE FROM post_categories WHERE post_id = ?', [post_id]);
    for (const catId of categoryIds) {
        run('INSERT OR IGNORE INTO post_categories (post_id, category_id) VALUES (?, ?)', [post_id, catId]);
    }
